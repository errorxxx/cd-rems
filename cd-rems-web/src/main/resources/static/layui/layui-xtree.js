...
layuiXtree.prototype.Initial = function (o) {
    ...
    _this._iconSelf = o.icon.iconSelf != null ? o.icon.iconSelf : '&#xe857;';
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            if (d[i].data.length > 0){
            	if(d[i].icon!=null&&d[i].icon!=''&&d[i].icon!=undefined){
            		_this._domStr += '<i class="layui-xtree-icon fa '+d[i].icon+'" data-xtree="' + (_this._isopen ? '1' : '0') + '"></i>';
            	}else{
            	...
            	if(d[i].icon!=null&&d[i].icon!=''&&d[i].icon!=undefined){
            		_this._domStr += '<i class="layui-xtree-icon-null fa '+d[i].icon+' "></i>';
            	}else if(d[i].menuType=='3'){
            		_this._domStr += '<i class="layui-icon layui-xtree-icon-null">' + _this._iconSelf + '</i>';
            	}else{
...
//获取全部选中的末级checkbox对象 
//cks[i].getAttribute('data-xend') == '1' 加上这个判断则不返回选中父节点
